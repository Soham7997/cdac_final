...
try:
    ...
    if hasattr(mp, 'solutions'):
        ...
        print("⚠ WARNING: MediaPipe loaded but solutions not available")
except (ImportError, AttributeError) as e:
    print(f"⚠ WARNING: MediaPipe error: {e}. Some features will be limited.")

# Create comprehensive placeholder for mp if not available
if not MEDIAPIPE_AVAILABLE:
    class PlaceholderDrawingSpec:
        def __init__(self, color=None, thickness=None, circle_radius=None):
            self.color = color
            self.thickness = thickness
            self.circle_radius = circle_radius
    
    class PlaceholderDrawingUtils:
        DrawingSpec = PlaceholderDrawingSpec
        
        @staticmethod
        def draw_landmarks(image, landmarks, connections, landmark_drawing_spec=None, connection_drawing_spec=None):
            return None
    
    class PlaceholderPoseLandmark:
        LEFT_SHOULDER = 11
        RIGHT_SHOULDER = 12
        LEFT_ELBOW = 13
        RIGHT_ELBOW = 14
    ...
    class PlaceholderHolistic:
        POSE_CONNECTIONS = []
        HAND_CONNECTIONS = []
        PoseLandmark = PlaceholderPoseLandmark
    ...
    class PlaceholderFaceMesh:
        FACEMESH_TESSELATION = []
    
    class PlaceholderSolutions:
        drawing_utils = PlaceholderDrawingUtils
        face_mesh = PlaceholderFaceMesh
        
        class holistic:
            Holistic = PlaceholderHolistic
            POSE_CONNECTIONS = []
            HAND_CONNECTIONS = []
            PoseLandmark = PlaceholderPoseLandmark
